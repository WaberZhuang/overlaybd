...
struct WarpFileArgs {
    ...
    photon::fs::IFile *lba_file = nullptr;  // lba pass from ioctl
    uint64_t virtual_size;
    UUID::String parent_uuid;
    UUID uuid;
    char *user_tag = nullptr; // a user provided string of message, 256B at most
    size_t len = 0;           // len of user_tag; if it's 0, it will be detected with strlen()
    WarpFileArgs(photon::fs::IFile *findex, photon::fs::IFile *fsmeta, photon::fs::IFile *target_file, photon::fs::IFile *lba_file)
...
extern "C" IFileRW *open_warpfile_rw(photon::fs::IFile *findex, photon::fs::IFile *fsmeta_file, photon::fs::IFile *lba_file,
