...
int ino_iter_extents(ext2_filsys fs, ext2_ino_t ino,
	...
	for (;;) {
		...
		if ((extent.e_flags & EXT2_EXTENT_FLAGS_SECOND_VISIT) ||
			...
			continue;
        LOG_INFO("found block ` `", extent.e_pblk, extent.e_len);
...
int ino_iter_blocks(ext2_filsys fs, ext2_ino_t ino,
				 std::vector<std::pair<off_t, size_t>> &blocks)
{
	struct ext2_inode inode;
	ext2_extent_handle_t extents;
	...
	if (retval) return parse_extfs_error(fs, ino, retval);

	if (!ext2fs_inode_has_valid_blocks2(fs, &inode)) {
        LOG_INFO("ext2fs_inode_has_valid_blocks2");
        return 0;
	...
	if (retval == EXT2_ET_INODE_NOT_EXTENT) {
        LOG_INFO("EXT2_ET_INODE_NOT_EXTENT");
        return 0;
...
int do_ext2fs_fallocate(ext2_filsys fs, ext2_ino_t ino, off_t offset, off_t len) {
    ...
    LOG_INFO("do_ext2fs_fallocate ", VALUE(offset), VALUE(len));
    auto start = offset / fs->blocksize;
    auto end = (offset + len - 1) / fs->blocksize;
    LOG_INFO("ext2fs_fallocate blocks `", end - start + 1);
...
int do_find_blocks(ext2_filsys fs, ext2_ino_t ino, std::vector<std::pair<off_t, size_t>> &blocks) {
    ino_iter_blocks(fs, ino, blocks);
    size_t total = 0;
    for (auto p: blocks) {
        // LOG_INFO(VALUE(p.first), VALUE(p.second));
        total += p.second;
    }
    total *= fs->blocksize;
    LOG_INFO(VALUE(total));
    return 0;
}
...
public:
    ...
    virtual int fiemap(struct photon::fs::fiemap* map) override {
        std::vector<std::pair<off_t, size_t>> blocks;
        do_find_blocks(fs, ino, blocks);
        map->fm_mapped_extents = blocks.size();
        photon::fs::fiemap_extent *ext_buf = &map->fm_extents[0];
        for (int i = 0; i < blocks.size(); i++) {
            LOG_INFO("find block ` `", blocks[i].first * fs->blocksize, blocks[i].second * fs->blocksize);
